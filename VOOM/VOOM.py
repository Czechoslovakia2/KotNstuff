...
class VOOMControlView(ui.View):
    """Persistent control view with three buttons."""
    def __init__(self, cog: "VigilOfOriginsMicrowaved"):
        super().__init__(timeout=None)
        self.cog = cog

    @ui.button(label="Recruit", style=discord.ButtonStyle.primary, custom_id="voom:recruit")
    ...
    @ui.button(label="Register", style=discord.ButtonStyle.secondary, custom_id="voom:register")
    ...
    @ui.button(label="Leaderboard", style=discord.ButtonStyle.success, custom_id="voom:leaderboard")
...
class VOOM(commands.Cog):
    ...
    def __init__(self, bot: Red):
        ...
        default_guild = {
            ...
            "auto_weekly_tz": "America/New_York",
            ...
            "defcon_levels": [  # evaluated by queue < lt, in ascending lt
                {"lt": 200,  "name": "Aurigids",   "emoji": "✨","reward":10},
                {"lt": 300,  "name": "Lyrids",    "emoji": "💫","reward":9},
                {"lt": 400,  "name": "Orionids",   "emoji": "🌠","reward":8},
                {"lt": 500,  "name": "Eta Aquariids",    "emoji": "☄️","reward":7},
                {"lt": 800,  "name": "Quadrantids", "emoji": "🎆","reward":6},
                {"lt": 1000, "name": "Geminids",    "emoji": "🎇","reward":5},
            ],
            "default_over_reward": 4,
            "defcon_overflow_name": "Perseids",
        ...
        self.bot.add_view(VOOMControlView(self))
    ...
    async def cog_load(self):
        ...
        if self.weekly_task is None or self.weekly_task.done():
            self.weekly_task = asyncio.create_task(self._weekly_scheduler(), name="VOOM_WeeklyPayout")
    ...
    async def start_listener(self):
        ...
        self.listener_task = asyncio.create_task(self._run_listener(), name="VOOM_SSE_Listener")
    ...
    async def handle_recruit(self, interaction: discord.Interaction):
       ...
       # Build list of up to 8 newest nations
        ...
        class RecruitView(discord.ui.View):
            def __init__(self, cog: "VOOM", link_url: str):
            ...
            @discord.ui.button(label="Remind in 40s", style=discord.ButtonStyle.secondary, row=1, custom_id="voom:remind:40")
            async def remind_40(self, interaction: discord.Interaction, button: discord.ui.Button):
                await self._schedule(interaction, 40)

            @discord.ui.button(label="Remind in 50s", style=discord.ButtonStyle.secondary, row=1, custom_id="voom:remind:50")
            async def remind_50(self, interaction: discord.Interaction, button: discord.ui.Button):
                await self._schedule(interaction, 50)

            @discord.ui.button(label="Remind in 60s", style=discord.ButtonStyle.secondary, row=1, custom_id="voom:remind:60")
            async def remind_60(self, interaction: discord.Interaction, button: discord.ui.Button):
                await self._schedule(interaction, 60)

            @discord.ui.button(label="Remind in 120s", style=discord.ButtonStyle.secondary, row=1, custom_id="voom:remind:120")
        ...
        await interaction.response.send_message(
            content=(
                ...
                f"Instant reward: **{instant_coins}** Funds\n\n"
                f"Targets: `{tgto}`"
            ),
            view=view,
            ephemeral=True,   
    ...
    # ---------- Commands ----------
    @commands.group(name="voom")
    @checks.admin_or_permissions(manage_guild=True)
    async def voom_group(self, ctx: commands.Context):
    ...
    @voom_group.command(name="setchannel")
    ...
    @voom_group.command(name="setuseragent")
    ...
    @voom_group.command(name="clearqueue")
    ...
    @voom_group.command(name="resetstats")
    ...
    def _build_control_embed(self, qlen: int, status_text: str, reward: int, level_name: str, level_emoji: str, idx: int, total: int) -> discord.Embed:
        embed = discord.Embed(
            title="The Launch Complex — Founding Monitor",
            color=discord.Color.blue(),
        )
        embed.add_field(name="Status", value=status_text, inline=False)
        embed.add_field(name="Queue", value=f"**{qlen} nations**", inline=False)
        embed.add_field(name="Current Reward", value=f"**{reward}** Funds per TG", inline=True)
    
        bar = self._level_bar(idx, total)
        embed.add_field(
            name="Meteor Shower Intensity",
    ...
    @commands.command(name="bumpvoom")
    @checks.admin_or_permissions(manage_guild=True)
    async def bump_voom(self, ctx: commands.Context):
    ...
    @voom_group.group(name="blacklist", invoke_without_command=True)
    @checks.admin_or_permissions(manage_guild=True)
    async def voom_blacklist(self, ctx: commands.Context):
    ...
    @voom_blacklist.command(name="add")
    async def voom_blacklist_add(self, ctx: commands.Context, *, region: str):
    ...
    @voom_blacklist.command(name="remove")
    async def voom_blacklist_remove(self, ctx: commands.Context, *, region: str):
    ...
    @voom_blacklist.command(name="clear")
    async def voom_blacklist_clear(self, ctx: commands.Context):
    ...
    @voom_group.command(name="setactiverole")
    @checks.admin_or_permissions(manage_guild=True)
    ...
    @voom_group.group(name="rewards", invoke_without_command=True)
    @checks.admin_or_permissions(manage_guild=True)
    ...
    @voom_group.command(name="setpotincrement")
    ...
    @voom_group.command(name="setminweekly")
    ...
    @voom_group.command(name="weeklypayout")
    ...
    @voom_group.command(name="autosettlement")
    ...
    @voom_group.command(name="testpayout")
    @checks.admin_or_permissions(manage_guild=True)
    async def test_payout(self, ctx: commands.Context, minutes: int = 5, tz: str = "America/Chicago"):
        ...
        Usage: [p]voom testpayout 5 America/Chicago
        """
    ...
    async def _schedule_reminder(self, interaction: discord.Interaction, seconds: int):
        ...
        asyncio.create_task(
            _job(),
            name=f"VOOM_Reminder_{guild.id if guild else 'dm'}_{user.id}_{seconds}",
        )
        ...
        if idx > last_idx and (now_ts - last_panic >= cooldown):
            if channel:
                try:
                    ...
                    await channel.send(
                        content=(
                            f"{role_mention}{emoji} **Meteor Shower Intensity has risen to {name}** "
                            f"({idx+1}/{total+1})\n"
    ...
    @voom_group.group(name="defcon", invoke_without_command=True)
    @checks.admin_or_permissions(manage_guild=True)
    async def defcon_group(self, ctx: commands.Context):
        """Show DEFCON-style Meteor Shower levels."""
        ...
        await ctx.send("**Meteor Shower Levels**\n" + "\n".join(lines))
    ...
    @voom_group.group(name="panic", invoke_without_command=True)
    @checks.admin_or_permissions(manage_guild=True)
    ...
    async def _notify_listener_error(self, note: str, exc: Optional[BaseException] = None, cooldown_sec: int = 120):
        ...
        for guild in self.bot.guilds:
            try:
                ...
                await owner.send(f"⚠️ **VOOM listener error** — {note}{snippet}")
            except Exception:
    ...
    @voom_group.command(name="setrecruiterrole")
    @checks.admin_or_permissions(manage_guild=True)
    ...
    @commands.command(name="voom_stats")
    async def voom_stats(self, ctx: commands.Context):
        """
        ...
        # --- Build embed ---
        embed = discord.Embed(
            title="⬡ Kerbals of the North Recruitment Stats ⬡",
